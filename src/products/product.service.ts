...
export class ProductService {
    ...
    getOrderProducts(email, orderNumber) {
        return this.orderProductModel.find({customer: email, orderNumber}).exec();
    }
    ...
    async createNewOrder(email: string) {

        // Fetch basket products for the given email
        const basketProducts = await this.getBasketProducts(email);

        if (basketProducts.length === 0) {
            throw new HttpException('The basket is empty', HttpStatus.BAD_REQUEST);
        }

        // Create an order number - necessary for grouping the products
        const orderNumber = Date.now();

        // Convert basket products to order products
        const orderProductsPromises = basketProducts.map(async (basketProduct) => {
            const orderProduct = new this.orderProductModel({
                ...basketProduct.toObject(),
            ...
            // Save the new order product
            return orderProduct.save();
        });

        // Wait for all order products to be saved
        const orderProducts = await Promise.all(orderProductsPromises);

        // Clear the basket
        await this.clearBasket(email);

        return orderProducts;

    }

    async clearBasket(email: string): Promise<any> {
        return this.basketProductModel.deleteMany({customer: email}).exec();
    }

    async requestOrderCancellation(email, orderNumber: number) {
        const orderProducts = await this.getOrderProducts(email, orderNumber)

        if (orderProducts.every(orderProduct => orderProduct.status === OrderStatus.pending)) {
            await this.basketProductModel.updateMany({
                customer: email,
                orderNumber: orderNumber
            }, {$set: {status: OrderStatus.canceled}}).exec()

            return; // Operation succeeded, no need to return anything
        }
        throw new HttpException(
            `Order ${orderNumber} has exceeded the time limit for free cancellation. Please contact customer support for more information.`,
